...
// ניסוחים של תשובות ה-AI של גוגל, מריצה אמיתית מול גולד פיש
const GOOGLE_REAL = [
  ['"העסק הבולט והמומלץ … הוא X"',
   'בביתר עילית, העסק הבולט והמומלץ לקניית דגים טריים הוא גולד פיש, הממוקמת ברחוב רבי עקיבא 23.', 3],
  ['אוגד קושר את ההמלצה גם ממרחק תשע מילים',
   'המומלץ המרכזי והבולט ביותר לקניית דגים טריים בביתר עילית הוא גולד פיש, קצביה וחנות בוטיק.', 3]
];
for (const [name, text, want] of GOOGLE_REAL) {
  eq(name, A.analyzeCell(text, CLIENT, RIVALS).status, want);
}
ok('בלי אוגד, מרחק גדול עדיין נדחה',
   A.analyzeCell('המומלץ המרכזי לקניית דגים טריים באזור ביתר עילית והסביבה נמצא ליד גולד פיש.',
                 CLIENT, RIVALS).status !== 3);
