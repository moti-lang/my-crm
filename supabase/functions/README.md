...
`_shared/answer-resolve.ts` — סדר העדיפות (מאגר גובר → מידע על החוג מעוגן → הפניה)
והשומרים (מחיר, הבטחות), בלי מסד. רץ גם בוואטסאפ וגם בסימולטור, כדי שהסימולטור
יראה בדיוק מה היה נשלח ומאיפה.
