...
### ⚠️ חובה לפני מסירה ללקוחה: החלפת הסיסמאות

הסיסמה `Teichtal!2026` כתובה בקוד ובקובץ הזה — כלומר בגיט, לכל מי שרואה
את המאגר. **אסור למסור מערכת שהמשתמשים בה עדיין נכנסים איתה.**

```bash
SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... npm run seed:rotate
```

`scripts/rotate-seed-passwords.mjs` מגריל סיסמה חזקה לכל אחד משלושת
המשתמשים, מחליף אותה דרך ה-Admin API, מוודא שהסיסמה הישנה כבר לא נכנסת,
ומדפיס את החדשות **פעם אחת בלבד**. הן לא נשמרות בשום מקום — למסור ללקוחה
בערוץ מאובטח ולמחוק מהמסוף. אם אבדו, מריצים שוב.

סיסמה שהלקוחה בחרה בעצמה נקבעת דרך משתני סביבה
(`SEED_PASSWORD_OWNER`, `SEED_PASSWORD_BRANCH_MANAGER`, `SEED_PASSWORD_ACCOUNTANT`),
לפחות 12 תווים. שלב 3 של סבב האימות מניח את סיסמת ה-seed, לכן הסדר הוא:
סבב אימות מלא ← החלפת סיסמאות ← מסירה.
