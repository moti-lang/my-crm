...
>   היא `claude-haiku-4-5`. `supabase/tests/model-benchmark.mjs` משווה
...
>   | `claude-haiku-4-5`  | 27/30 (90%) | 17/17 (100%) | 12/13 (92%) | 56/60 |
>
>   הסכומים מדויקים ב-100% בשלושת המודלים, וזה השדה שטעות בו הכי יקרה.
>   הפרשי הדיוק (56/57/59 מתוך 60) אינם משמעותיים, והם כולם בקלט זבל
>   ("שילמתי", "מה המצב") שנדחה ממילא בסף הביטחון או בכרטיס האישור.
...
>   | מודל | זמן | דיוק |
>   |---|---|---|
>   | `claude-haiku-4-5`  | **~1,700ms** (1669 · 1702 · 1689) | 56/60 |
>   | `claude-sonnet-4-6` | 2,500-2,900ms, ולעיתים מעל 4,000 | 57/60 |
>   | `claude-opus-5`     | לא נמדד לזמן | 59/60 |
>
>   **ההכרעה: `claude-haiku-4-5`.** הוא היחיד שעומד בסף 3 השניות בוודאות,
>   והפער בעקביות שלו בולט (33ms בין שלוש קריאות). המחיר הוא נקודה אחת
>   מתוך 60, בקלט זבל.
>   `ANTHROPIC_MODEL` נשאר משתנה — אם יתגלו טעויות בשטח, מחליפים בלי
>   נגיעה בקוד.
...
> **כלל קבוע — בדיקה לא מחפשת מחרוזת שיכולה להופיע בהערה.**
> זה קרה שש פעמים, לשני הכיוונים: בדיקה חיובית שעברה כי המילה שרדה
> בהערה אחרי שהקוד נמחק, ובדיקה שלילית שנכשלה כי הערה הזכירה את מה
> שאסור להיות בקוד. כל קריאת מקור בבדיקה עוברת דרך `_code.mjs` —
> `codeOf` (בלי הערות) או `rawOf` (מפורש, כשהטקסט עצמו נבדק).
> `test-hygiene.test.mjs` אוכף את זה ובודק גם את המסיר עצמו.
...
| AI | Claude API דרך Edge Function בלבד. המודל ב-`ANTHROPIC_MODEL`, ברירת מחדל `claude-haiku-4-5` |
| גרפים | Recharts |
